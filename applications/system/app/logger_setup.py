...
app.logger.removeHandler(app.config['TIMEZONE'])
...
def add_fields(_, level, event_dict):
    ...
    now = dt.datetime.now()
    event_dict['timestamp'] = TZ.localize(now, True).astimezone(pytz.utc).isformat()
...
# Add a handler to write log messages to file
